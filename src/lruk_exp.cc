...
RunStatistics run_policy(Loader& loader, size_t capacity, Policy&& policy)
{
	BufPoolRunner<Policy> runner{capacity, policy};
...
RunStatistics run_policy(TraceView traces, size_t capacity, Policy&& policy)
{
	SpanLoader loader{traces};
	return run_policy<Policy, SpanLoader>(loader, capacity, policy);
}
...
void run_lruk_trace(TraceView trace, size_t capacity)
{
	...
	auto run_lru_config = [&](size_t grace, double recency_pct) {
		{
			LRUKNoRecencyPolicy<K> policy{capacity, grace, recency_pct};
			auto stats =
			    run_policy<LRUKNoRecencyPolicy<K>>(trace, capacity, std::move(policy));
			print_stats_row(std::format("LRU{} n/rec {} {}", K, grace, recency_pct),
			                stats);
		}
		{
			LRUKWithRecencyPolicy<K> policy{capacity, grace, recency_pct};
			auto stats =
			    run_policy<LRUKWithRecencyPolicy<K>>(trace, capacity, std::move(policy));
			print_stats_row(std::format("LRU{} w/rec {} {}", K, grace, recency_pct),
			                stats);
		}
	};
